...
// Normalize region string for matching (trim, uppercase)
function matchRegion(value) {
  if (value == null || value === '') return null;
  const s = String(value).trim();
  return REGIONS.find((r) => r.trim().toUpperCase() === s.toUpperCase()) || null;
}
...
router.post('/', upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // FIRST: Look for exact "Region" column (case-insensitive)
      const regionKey = Object.keys(row).find((k) => String(k).toLowerCase().trim() === 'region');
      if (regionKey) {
