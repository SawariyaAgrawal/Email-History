...
Backend: Node.js + Express + MongoDB. Frontend: HTML, CSS, vanilla JavaScript.  
...
1. **MongoDB**  
   Install and run MongoDB locally (e.g. `mongod`), or set `MONGODB_URI` in `.env`.

2. **Backend**
   ```bash
   cd backend
   cp .env.example .env
   # Edit .env: MONGODB_URI, JWT_SECRET, optional PORT and MAX_FILE_SIZE_MB
   npm install
...
- **records** – one document per Excel row; dynamic fields in `data`; `uploadedBy`, `sourceFile`, `rowIndex`; unique `_id` for detail view.
